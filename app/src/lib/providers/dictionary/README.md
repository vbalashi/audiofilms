...
Unauthenticated AudioFilms `/api/dict/lookup` requests use
`POST /api/platform/v1/catalog/lookup` when
`DICTIONARY_2000NL_CATALOG_ACCESS_TOKEN` is configured. Catalog lookup is
read-only and does not project 2000NL progress actions.
...
State-aware dictionary cards require a forwarded user Bearer token. In that
mode AudioFilms calls `POST /api/platform/v1/lookup` with
`includeUserState:true` and `intent:"external-click"`.
