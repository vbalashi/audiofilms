...
- `phrases`: timed phrase-like units with `id`, `startSec`, `endSec`, and `text`.
...
The app's canonical practice phrase normalizer lives in `app/src/lib/practice/phrases.ts`.
...
The extension mirrors these defaults in `extensions/youtube-shadowing/src/phrases.js`.
...
- The extension cannot import the app normalizer yet because it has no bundling step.
